...
import { MonitorUp, Pin, Maximize, Minimize } from 'lucide-vue-next'
...
<template>
  <div class="flex-1 p-4 overflow-auto flex flex-col gap-3 bg-gray-100 dark:bg-gray-900">
    ...
    <template v-if="focusedSid">
      <div
        v-for="{ participant, isLocal } in participants.filter(p => p.participant.sid === focusedSid)"
        :key="'focused-' + participant.sid"
        :id="`tile-${participant.sid}`"
        class="relative bg-white dark:bg-gray-800 rounded-xl overflow-hidden flex-1 min-h-0 border border-gray-200/80 dark:border-white/[0.06] shadow-card dark:shadow-card cursor-pointer"
      ...
      >
        ...
        <div
          ...
          :class="screenShares.some(s => s.identity === participant.identity)
            ? 'absolute bottom-3 right-3 w-pip-w h-pip-h rounded-lg overflow-hidden shadow-xl border border-gray-700/50'
            : 'absolute inset-0'"
        ></div>
        <!-- Avatar fallback -->
        ...
        <TileNameBar
          ...
          :mic-enabled="micEnabled" :cam-enabled="camEnabled"
          :is-sharing="screenShares.some(s => s.identity === participant.identity)"
        ...
        <div class="absolute top-2 right-2 flex gap-1 z-30">
          <AppTooltip :content="t('chat.pin')" position="bottom">
            <button @click.stop="emit('pin', participant.sid)" class="bg-black/60 hover:bg-black/80 rounded p-1.5 cursor-pointer transition-colors">
          ...
          <AppTooltip :content="fullscreenSid === participant.sid ? t('chat.exitFullscreen') : t('chat.fullscreen')" position="bottom">
            <button @click.stop="emit('fullscreen', participant.sid)" class="bg-black/60 hover:bg-black/80 rounded p-1.5 cursor-pointer transition-colors">
      ...
      <div class="flex gap-2 shrink-0">
        <div
          ...
          :id="`tile-${participant.sid}`"
          class="relative w-pip-w h-pip-h bg-white dark:bg-gray-800 rounded-lg overflow-hidden border border-gray-200/80 dark:border-white/[0.06] cursor-pointer hover:ring-2 hover:ring-indigo-400 transition-all"
          :class="activeSpeakers.has(participant.identity) ? 'ring-2 ring-green-400 shadow-glow-speaker' : ''"
          @click="emit('focus', participant.sid)"
        >
          <div :id="`screen-share-${participant.sid}`" class="absolute inset-0 z-10"></div>
          <div
            ...
            class="z-20"
            :class="screenShares.some(s => s.identity === participant.identity)
              ? 'absolute bottom-1 right-1 w-10 h-8 rounded overflow-hidden border border-gray-700/50'
              : 'absolute inset-0'"
          ></div>
      ...
      <div class="flex flex-col gap-2 w-strip-w shrink-0">
        ...
        >
          ...
          <div class="absolute top-1 right-1 flex gap-1 z-30 opacity-0 group-hover:opacity-100 transition-opacity">
            <AppTooltip :content="t('chat.pin')" position="bottom">
              <button @click="emit('pin', participant.sid)" class="bg-black/50 hover:bg-black/70 rounded p-0.5 cursor-pointer">
    ...
    >
      <div
        ...
        :key="participant.sid"
        :id="`tile-${participant.sid}`"
        class="relative bg-white dark:bg-gray-800 rounded-xl overflow-hidden min-h-grid-min transition-all duration-300 group border border-gray-200/80 dark:border-white/[0.06] shadow-card dark:shadow-card cursor-pointer"
        :class="activeSpeakers.has(participant.identity) ? 'ring-2 ring-green-400 shadow-glow-speaker' : ''"
        @click="emit('focus', participant.sid)"
      >
        <!-- Screen share full card -->
        ...
        <div class="absolute top-2 right-2 flex gap-1 z-30 opacity-0 group-hover:opacity-100 transition-opacity">
          <AppTooltip :content="t('chat.pin')" position="bottom">
            <button @click.stop="emit('pin', participant.sid)" class="bg-black/50 hover:bg-black/70 rounded p-1 cursor-pointer">
              <Pin class="w-3.5 h-3.5 text-white" :stroke-width="2" />
            </button>
          </AppTooltip>
          <AppTooltip :content="fullscreenSid === participant.sid ? t('chat.exitFullscreen') : t('chat.fullscreen')" position="bottom">
            <button @click.stop="emit('fullscreen', participant.sid)" class="bg-black/50 hover:bg-black/70 rounded p-1 cursor-pointer">
